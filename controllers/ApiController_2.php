    ...
    public function actionGetsearchresult($callback = null) {
        ...
        $connection = Yii::$app->db;
            if($location){
                ...
                $query  = "SELECT `id`,`name`,`propertyid`,`location`,`city`,`path`,`persquarefeet`,`propertyarea`,`ownername`,`landmark`,`ameneties`,`expectedprice`,`resellrent`,`shoporoffice`,`typeofproperty`,`selectproperty`,`selecttype`,`status` from searchproperty "
                        . "WHERE selectproperty = '$propertyres' "
                        . "AND selecttype = '$propertynew' "
                        . "|| selectproperty = $propertybhk "
                        . "AND (location LIKE '%$search%') "
                        . "|| (city LIKE '%$search%') "
                        . "AND `expectedprice` BETWEEN $minprice AND $maxprice";
...
//        echo "<>pre";die;
    ...
    public function actionGetsearchresidentialresale($callback = null) {
        ...
        $connection = Yii::$app->db;
        // echo
        $query = "SELECT * from resaleproperty WHERE (location LIKE '$location%')|| (location LIKE '%$location') || (location LIKE '%$location%') AND  projecttype ='Residential' AND resellrent ='Resale'  AND (expectedprice BETWEEN $minprice AND $maxprice) AND selecttype = $type";
        // die;
        $objData = $connection->createCommand($query)->queryAll();
        foreach ($objData as $key => $value) {
