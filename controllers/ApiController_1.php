...
//              $umesh_sir = "umeshs@uniquepaf.com";
    ...
    public function actionGetbuilderinfobyid($callback = null) {
        $arrReturn = array();
        $arrReturn['status'] = FALSE;
        $request = Yii::$app->request;
        $id = $request->get('id');
        $objData = Builder::find()->where(['id' => $id, 'status' => 'Active'])->one();
        if ($objData) {
            $arrReturn['status'] = TRUE;
            $arrReturn['data'] = $objData;
        }
    ...
    public function actionGetprojectimagesbytype($callback = null) {
        $arrReturn = array();
        $arrReturn['status'] = FALSE;
        $request = Yii::$app->request;
        $resprojectid = $request->get('id');
    ...
    public function actionGetresproject($callback = null) {
        ...
        $objData = Resproject::find()
                        ->andFilterWhere([
                            'and',
                            ['like', 'city', $city],
                            ['like', 'location', $location],
                        ])->andWhere('status = :status', [':status' => $status])->orderBy([ 'added_date' => SORT_DESC])->all();
        $objCity = Resproject::find()->select('city')->distinct()->all();
        $objLocation = Resproject::find()->select('location')->distinct()->all();
        $arrReturn['city'] = $objCity;
        $arrReturn['location'] = $objLocation;
...
// ????
    ...
    public function actionGetsearchresidentialnew($callback = null) {
        $arrReturn = array();
        $arrReturn['status'] = FALSE;
        $request = Yii::$app->request;
        $status = 'Active';
        $propertyid = $request->get('propertyid');
        $typeofproperty = $request->get('typeofproperty');
...
//      $query = "SELECT * from searchproperty WHERE (CONCAT(`location`, `city`) LIKE '%$location%') || (CONCAT(`propertyid`) LIKE '%$propertyid%') || totalcharges BETWEEN $minprice AND $maxprice AND  typeofproperty $typeofproperty";
//        $query = "SELECT * from searchproperty WHERE (CONCAT(`location`, `city`) LIKE '%$search%') || (CONCAT(`propertyid`) LIKE '%$propertyid%') AND `totalcharges` BETWEEN $minprice AND $maxprice AND (typeofproperty LIKE '%$typeofproperty')";
        $query = "SELECT * from searchproperty WHERE (location LIKE '$search%')|| (location LIKE '%$search') || (location LIKE '%$search%') || (city LIKE '$search%')|| (city LIKE '%$search') || (city LIKE '%$search%') || (propertyid LIKE '%$propertyid%') AND `totalcharges` BETWEEN $minprice AND $maxprice || (typeofproperty LIKE '%$typeofproperty')";
...
//                foreach ($objData as  $value) {
                    $arrTemp = array();
                    $arrTemp['project'] = $objData;
    //                $arrTemp['property'] = Resprojectproperty::find()->where(['id' => $value['resprojectpropertyid']])->all();
    //                $arrTemp['builder'] = Builder::find()->where(['id' => $value['builderid']])->all();
    //                $arrTemp['amaneties'] = Resprojectamaneties::find()->where(['id' => $value['resprojectamanetiesid']])->all();
    //                $arrTemp['cost'] = Resprojectcost::find()->where(['id' => $value['resprojectcostid']])->all();
    //                $arrTemp['resprojectimage'] = Resprojectimage::find()->where(['resprojectid' => $value['id']])->all();
    //                if (!$arrTemp['resprojectimage']) {
    //                    $arrTemp1 = array();
    //                    $arrTemp1['path'] = '/resources/resproject/no-thumb.jpg';
...
//                }
    ...
    public function actionGetresprojectcost($callback = null) {
        $arrReturn = array();
        $arrReturn['status'] = FALSE;
        $request = Yii::$app->request;
        $resprojectid = $request->get('resprojectid');
        $objData = Resproject::find(['id' => $resprojectid])->one();
        if($objData){
            $arrReturn['status'] = TRUE;
            $arrReturn = array();
    ...
    public function actionGetcomprojectcost($callback = null) {
        $arrReturn = array();
        $arrReturn['status'] = FALSE;
        $request = Yii::$app->request;
        $comprojectid = $request->get('comprojectid');
        $objData = Comproject::find(['id' => $comprojectid])->one();
        if($objData){
            $arrReturn['status'] = TRUE;
            $arrReturn = array();
            $arrReturn['name'] = $objData->name;
            $arrReturn['cost'] = $objData->comprojectcost->toArray();
        }
            $arrReturn['data'][] = $arrReturn;
        // set "fomat" property
        Yii::$app->getResponse()->format = (is_null($callback)) ?
    ...
    public function actionGetcomconstructiondata($callback = null) {
        ...
        if($objData){
            ...
            $arrReturn = array();
            $arrReturn['name'] = $objData->name;
            $arrReturn['comprojectproject'] = $objData->comprojectproject->toArray();
        }
            $arrReturn['data'][] = $arrReturn;
        // set "fomat" property
        Yii::$app->getResponse()->format = (is_null($callback)) ?
    ...
    public function actionGetsearchresidentialresale($callback = null) {
        ...
        $connection = Yii::$app->db;
        // echo
        $query = "SELECT * from resaleproperty WHERE (location LIKE '$location%')|| (location LIKE '%$location') || (location LIKE '%$location%') AND  projecttype ='Residential' AND resellrent ='Resale'  AND (expectedprice BETWEEN $minprice AND $maxprice)";
        // die;
        $objData = $connection->createCommand($query)->queryAll();
        foreach ($objData as $key => $value) {
    ...
    public function actionGetsearchresidentialrent($callback = null) {
        ...
        $connection = Yii::$app->db;
        // echo
        $query = "SELECT * from resaleproperty WHERE (location LIKE '$location%')|| (location LIKE '%$location') || (location LIKE '%$location%') AND  projecttype ='Residential' AND resellrent ='Rent' AND (expectedprice BETWEEN $minprice AND $maxprice)";
        // die;
        $objData = $connection->createCommand($query)->queryAll();
    ...
    public function actionGetsearchcommercialresale($callback = null) {
        ...
        $connection = Yii::$app->db;
        // echo
        $query = "SELECT * from resaleproperty WHERE (location LIKE '$location%')|| (location LIKE '%$location') || (location LIKE '%$location%') AND  projecttype ='Commercial' AND resellrent ='Resale' AND (expectedprice BETWEEN $minprice AND $maxprice)";
        // die;
        $objData = $connection->createCommand($query)->queryAll();
    ...
    public function actionGetsearchcommercialrent($callback = null) {
        ...
        $connection = Yii::$app->db;
        // echo
        $query = "SELECT * from resaleproperty WHERE (location LIKE '$location%')|| (location LIKE '%$location') || (location LIKE '%$location%') AND  projecttype ='Commercial' AND resellrent ='Rent' AND (expectedprice BETWEEN $minprice AND $maxprice)";
        // die;
        $objData = $connection->createCommand($query)->queryAll();
    ...
    public function actionGetresproperty($callback = null) {
        ...
        $city = $request->get('city');
        $location = $request->get('location');
        $objData = Resproperty::find()
                        ->andFilterWhere([
                            'and',
                            ['like', 'city', $city],
                            ['like', 'location', $location],
                        ])->andWhere('status = :status', [':status' => $status])->all();
        $objCity = Resproperty::find()->select('city')->distinct()->all();
        $objLocation = Resproperty::find()->select('location')->distinct()->all();
        $arrReturn['city'] = $objCity;
        $arrReturn['location'] = $objLocation;
        foreach ($objData as $key => $value) {
            $arrTemp = array();
    ...
    public function actionGetcomproject($callback = null) {
        ...
        $city = $request->get('city');
        $location = $request->get('location');
        $objData = Comproject::find()
                        ->andFilterWhere([
                            'and',
                            ['like', 'city', $city],
                            ['like', 'location', $location],
                        ])->andWhere('status = :status', [':status' => $status])->orderBy([ 'added_date' => SORT_DESC])->all();
        $objCity = Comproject::find()->select('city')->distinct()->all();
        $objLocation = Comproject::find()->select('location')->distinct()->all();
        $arrReturn['city'] = $objCity;
        $arrReturn['location'] = $objLocation;
        foreach ($objData as $key => $value) {
            $arrTemp = array();
    ...
    public function actionGetsearchcommercialnew($callback = null) {
        ...
        // $objData = Resproject::find()
        //                 ->andFilterWhere([
        //                     'or',['like', 'city', $location],['like', 'location', $location]])
        //     ->andFilterWhere(['between', 'expectedprice',$minprice,$maxprice])->andWhere('status = :status', [':status' => $status]) ->all();
