...
<div class="container">
    ...
    <div class="tab-content">
        <div role="tabpanel" class="tab-pane active" id="profile"> <!-- /tab-panel -list -->
            <h2>Fixed Deposit</h2>
                   <?php
                        foreach ($objFinancialbigcontent as $key => $value ) {
                            if($value->id===1 ){
                                 echo' <textarea id="content" value=""  name="content" >'.$value->content.' </textarea>';
                            }
                        }
                   ?>
        ...
        <div role="tabpanel" class="tab-pane" id="professional"><!-- /tab-panel settings -->
            <h2>Mutual Funds</h2>
                    <?php
                        foreach ($objFinancialbigcontent as $key => $value ) {
                            if($value->id===2 ){
                                 echo' <textarea id="mutualfunds" name="mutualfunds" >'.$value->content.' </textarea>';
                            }
                        }
                   ?>
        ...
        <div role="tabpanel" class="tab-pane" id="password"><!-- /tab-panel settings -->
            <h2>Bond</h2>
                  <?php
                        foreach ($objFinancialbigcontent as $key => $value ) {
                            if($value->id===4 ){
                                 echo'  <textarea  id="bond" name="bond">'.$value->content.' </textarea>';
                            }
                        }
                   ?>
         ...
         <div role="tabpanel" class="tab-pane" id="mediclaim1"><!-- /tab-panel settings -->
            <h2>Mediclaim</h2>
                   <?php
                        foreach ($objFinancialbigcontent as $key => $value ) {
                            if($value->id===3 ){
                                 echo'<textarea  id="mediclaim" name="mediclaim">'.$value->content.' </textarea>';
                            }
                        }
                   ?>
        ...
        <div role="tabpanel" class="tab-pane" id="insurance1"><!-- /tab-panel settings -->
            <h2>Insurance</h2>
                    <?php
                        foreach ($objFinancialbigcontent as $key => $value ) {
                            if($value->id===5 ){
                                 echo' <textarea  id="insurance" name="insurance">'.$value->content.' </textarea>';
                            }
                        }
                   ?>
         ...
         <div role="tabpanel" class="tab-pane" id="Personal"><!-- /tab-panel settings -->
            <h2>Personal Provident Fund</h2>
                  <?php
                        foreach ($objFinancialbigcontent as $key => $value ) {
                            if($value->id===6 ){
                                 echo'<textarea id="ppf" name="Personal">'.$value->content.' </textarea>';
                            }
                        }
                   ?>
...
function saveFixeddeposit() {
    if(confirm('Are you sure you want to change the Fixed deposit content?')){
        ...
        $.ajax({
            url: 'index.php?r=financialservices/savefixeddepositlargecontent',
            async: false,
            data: obj,
            type: 'POST',
    ...
    var nicE = new nicEditors.findEditor('mutualfunds');
            ...
            obj.content = nicE.getContent();
            $.ajax({
                url: 'index.php?r=financialservices/savemutualfundslargecontent',
                async: false,
                data: obj,
                type: 'POST',
                success: function(data) {
     ...
     function saveMediclaim() {
       ...
       var nicE = new nicEditors.findEditor('mediclaim');
            ...
            obj.content = nicE.getContent();
            $.ajax({
                url: 'index.php?r=financialservices/savemediclaimlargecontent',
                async: false,
                data: obj,
                type: 'POST',
                success: function(data) {
    ...
    var nicE = new nicEditors.findEditor('bond');
            ...
            obj.content = nicE.getContent();
            $.ajax({
                url: 'index.php?r=financialservices/savebondlargecontent',
                async: false,
                data: obj,
                type: 'POST',
                success: function(data) {
    ...
    var nicE = new nicEditors.findEditor('insurance');
            ...
            obj.content = nicE.getContent();
            $.ajax({
                url: 'index.php?r=financialservices/saveinsurancelargecontent',
                async: false,
                data: obj,
                type: 'POST',
                success: function(data) {
    ...
    var nicE = new nicEditors.findEditor('ppf');
            ...
            obj.content = nicE.getContent();
            $.ajax({
                url: 'index.php?r=financialservices/saveppflargecotent',
                async: false,
                data: obj,
                type: 'POST',
                success: function(data) {
